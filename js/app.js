...
function persist({ silent = false } = {}) {
  ...
  saveTimer = setTimeout(() => {
    try {
      ...
      alert("The notebook could not be saved. Large images can exceed browser localStorage limits. We will move this to IndexedDB in a later phase.");
    }
  }, 180);
...
function renderHome() {
  ...
  const usedFolderIds = new Set(state.notes.map(note => note.folderId).filter(Boolean));

  if (folders.length === 0) {
    const empty = document.createElement("div");
    empty.className = "empty";
    empty.innerHTML = "<strong>No folders yet.</strong><br>Create a folder or a note to get started.";
  ...
  for (const folder of folders) {
    const notes = state.notes.filter(note => note.folderId === folder.id && visibleNotes(note));

    if (query && !notes.length && !folder.name.toLowerCase().includes(query)) continue;
    ...
    title.innerHTML = `<span>📁</span><span>${escapeHtml(folder.name)}</span>`;

    const folderActions = document.createElement("div");
    folderActions.innerHTML = `
      <button class="more-btn" title="Folder options">⋮</button>
    `;
    folderActions.querySelector("button").addEventListener("click", () => folderMenu(folder));
    ...
    notes.forEach(note => notesWrap.appendChild(noteCard(note)));
  ...
  const unfiled = state.notes.filter(note => !note.folderId && visibleNotes(note));
  if (unfiled.length || (!state.notes.length && !state.folders.length)) {
    ...
    const head = document.createElement("div");
    head.className = "folder-head";
    head.innerHTML = `<div class="folder-title"><span>🗂️</span><span>Unfiled</span></div>`;

    const wrap = document.createElement("div");
    wrap.className = "folder-notes";
    unfiled.forEach(note => wrap.appendChild(noteCard(note)));
...
function noteCard(note) {
  ...
  open.innerHTML = `
    ...
    <span class="note-info">
      ...
      <span class="note-meta">${note.pages.length} page${note.pages.length === 1 ? "" : "s"} · ${formatDate(note.updatedAt)}</span>
    </span>
  `;
...
function openNote(noteId) {
  ...
  // Reopen at the last place without a page-click.
  requestAnimationFrame(() => editor.restorePosition());
}
...
function renameNote(note) {
  ...
  persistNow();
  renderHome();
  if (currentNote?.id === note.id) els.editorTitle.textContent = note.title;
}
...
function renameFolder(folder) {
  ...
  persistNow();
  renderHome();
  if (currentNote) els.editorFolder.textContent = `📁 ${folderName(currentNote.folderId)}`;
}
...
function moveNote(note) {
  ...
  const names = folders.map((folder, i) => `${i + 1}. ${folder.name}`).join("\n");
  const choice = Number(prompt(`Move "${note.title}" to:\n\n${names}\n\nEnter folder number:`));
...
function deleteNote(note) {
  if (!confirm(`Delete "${note.title}" and all its pages? This cannot be undone.`)) return;

  if (currentNote?.id === note.id) showHome();
...
function openModal({ title, message = "", placeholder = "", confirmText = "Create", showFolder = false, folders = [] }) {
...
els.modalInput.addEventListener("keydown", event => {
  if (event.key === "Enter") els.modalConfirm.click();
...
els.newFolder.addEventListener("click", async () => {
  const result = await openModal({
    title: "Create Folder",
    message: "Use folders to keep subjects, projects, or personal notes organized.",
...
els.renameNote.addEventListener("click", () => {
  if (currentNote) renameNote(currentNote);
});
...
document.addEventListener("paste", async event => {
  if (!editor || els.editor.classList.contains("hidden")) return;
  const handled = await editor.pasteImage(event);
  if (handled) event.preventDefault();
});

window.addEventListener("beforeunload", () => {
  if (editor && currentNote) editor.savePosition();
  persistNow();
});

window.addEventListener("resize", () => {
  // Canvas instances observe their own size.
});
